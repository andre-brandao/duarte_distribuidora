...
function createPedidoStore() {
	...
	function removeUmItemPedido(var_produto_id: number) {
		update((array_pedidos) => {
			...
			if (exists !== -1 && array_pedidos[exists].quantidade === 1) {
	...
	return { set, subscribe, update, addItemPedido, removeTodosItemPedido, removeUmItemPedido };
}

export let pedidoStore = createPedidoStore();
